...
const facilityOptions = {
  ...
  data: {
    ...
    address: {
      ...
      phoneNumbers: [
        {
          value: "066 - 66 66 66 66",
          type: "PHONE",
          label: "Direct Connection",
        },
      ],
      emailAddresses: [
        { value: "tim@mordornovelties.com", recipient: "Tim Otter" },
      ],
    },
    contact: {
      firstName: "Tim",
    ...
    pickingTimes: {
      ...
      saturday: [
        { end: { hour: 23, minute: 0 }, start: { hour: 0, minute: 0 } },
      ],
...
const modifyPickJobReqOpts1 = {
  ...
  data: {
    version: 1,
    actions: [{ action: "ModifyPickJob", status: "IN_PROGRESS" }],
  },
};
...
axios
  ...
  .then(function (response) {
    ...
    // create a facility
    axios
      .request(facilityOptions)
      .then(function (response) {
        console.log(response.data);
        // update request options for listing and order
        createListingReqOpts.url = `https://api-gateway-qhqutz5o2a-ew.a.run.app/api/facilities/${response.data.id}/listings`;
        createOrderReqOpts.data.deliveryPreferences.supplyingFacilities.push(
          response.data.id
        );
        createOrderReqOpts.data.deliveryPreferences.collect[0].facilityRef = `${response.data.id}`;
        createOrderReqOpts.data.deliveryPreferences.collect[0].supplyingFacilities.push(
          response.data.id
        );
        ...
        // create listing
        axios
          .request(createListingReqOpts)
          .then(function (response) {
            console.log("Listings created: " + response.status);
            // create order
            axios
              .request(createOrderReqOpts)
              .then(function (response) {
